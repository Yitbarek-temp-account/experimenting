...
export class AppointmentsService {
  ...
  async createAppointment(createAppointmentDto: CreateAppointmentDto): Promise<any> {
    ...
    const appointment = await this.appointmentsRepository.createOne(createAppointmentDto);
    return appointment;
  }
  ...
  async checkAppointment(doctor_id: string, active: boolean): Promise<any> {
    const findActiveAppointments = await this.appointmentsRepository.findAll(doctor_id, active);
  ...
  async acceptAppointment(appointment_id: string): Promise<any> {
    ...
    const acceptedAppointment = await this.appointmentsRepository.updateOne(findAppointment._id);
    ...
    const user = await this.usersService.updateAppointment(findAppointment.user, findAppointment._id);
    const acceptDoctorAppointment = await this.doctorsService.updateAppointment(findAppointment.doctor, findAppointment._id);
    const addCronNotification = await this.notificationsService.createCronNotification(
      findAppointment._id,
  ...
  async declineAppointment(appointment_id: string): Promise<any> {
    ...
    const removeAppointment = await this.appointmentsRepository.removeOne(findAppointment._id);
